    ...
    try {
        ...
        await connection.query(query, [email], async (error, response) => {
            ...
            await connection.query(('INSERT INTO users (name, email, password, role_id) VALUES (?, ?, ?, ?)'),
                [name, email, bcryptPassword, 1], (error, response) => {
                    if (error) return sendError(res, error, 400)

                    res.json({ success: true, msg: "User Added Successfuly", data: response, status: 200 })
                })
        });
    } catch (error) {
...
exports.login = async (req, res) => {
    ...
    try {
        ...
        await connection.query(query, [email], async (error, response) => {
            ...
            if (response.length === 0) return sendError(res, 'User Does not Exist', 401);
            ...
            res.json({
                success: true, msg: "Login Success", data: { token: token, role_id: response[0].role_id }
            });
        });
    } catch (error) {
        console.log(error.message);
